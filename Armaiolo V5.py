...
def trovaArma(armeria, arma):
    for i in armeria:
        if armeria == miaArmeria:
            if i['Dettagli']['Nome'] == arma:
                return i 
            else:
                print("Il negozio non contiene un arma con questo nome! ")
        elif armeria == negozio:
            for x, y in i.items():
...
M4 = Arma(prezzo = 700, modello = 'M4', precisione = 80, tipo = 'fucile automatico', calibro = 5.56, colpi = 30, paese = 'U.S.A.')
SPAS12 = Arma(prezzo = 1000, modello = 'Spas 12', precisione = 50, tipo = 'fucile a pompa', calibro = 12, colpi = 8, paese = 'Italia')
RPG7 = Arma(prezzo = 350, modello = 'RPG-7', precisione = 60, tipo = 'lanciamissili', calibro = 40, colpi = 1, paese = 'Russia')
BrowningM2 = Arma(prezzo = 12000, modello = 'Browning M2', precisione = 70, tipo = 'mitragliatrice pesante', calibro = 12.7, colpi = 250, paese = 'U.S.A.')
M79 = Arma(prezzo = 370, modello = 'M79', precisione = 50, tipo = 'lanciagranate', calibro = 40, colpi = 1, paese = 'U.S.A.')
# ------------------------------------------------------------------------------------
...
while repeat == True:
    ...
    if azione == 'e':
        ...
        print('')
        printArmeria(armiModificate, 'La tua rastrelliera delle armi modificate')
        print('')
    elif azione == '5':
        ...
        if armaMod is None:
            ...
            for x, y in modifiche.items():
                if x == mod:
                    d = copy.deepcopy(armaMod)
                    d['Quantità'] = 1
                    d['Dettagli']['Modifiche'] = []
                    d['Dettagli']['Modifiche'].append(mod)
                    d['Dettagli']['Precisione'] += y
                    armiModificate.append(d)
                    print(f"Modifica '{mod}' applicata con successo all'arma '{arma2}'!")
                    trovato = True
                    break
            if not trovato:
